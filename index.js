...
async function startjobotz() {
    const jobotz = jobotzConnect({
        ...
        browser: ['WXZ BOT V1', 'Safari','1.0.0'],
        auth: state
    })
    ...
    jobotz.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                asa = await getBuffer(`http://hadi-api.herokuapp.com/api/tts?text=Selamat Datang Kawan Jangan Lupa Subscribe Channel De Jo Bot Dan Fariz Mods Dan The Ghizy Juga Biar Emaknya Bangga&language=id`)
...
╟⊱❁ THE GHIZY
╟⊱❁ https://youtube.com/channel/UCdPcXlnmiVQf1Yvh3WQFRog
╟⊱❁ THE JO BOT
╟⊱❁ https://youtube.com/channel/UC-wt99jFVc-zXMkxKRDZ56w
╟⊱❁ Room VirusXBug Onlly
╟⊱❁ https://chat.whatsapp.com/F2qCcvcMmUc1A3QzwQLFxD
╰━ ━ ━ ━ ━ ━ ━ ━ ━ ━•⩵꙰ཱི࿐
 ━━━━━━━ ◦ ❖ ◦ ━━━━━━━

© BOT PRIBADI GHIZY
  `
                  jobotz.sendMessage(anu.id, { image: { url: pp_user }, contextInfo: { mentionedJid: [num] }, caption: anunya })
                  jobotz.sendMessage(anu.id, {audio: asa, mimetype: 'audio/mpeg', ptt:true })
                } else if (anu.action == 'remove') {
                asa1 = await getBuffer(`http://hadi-api.herokuapp.com/api/tts?text=Selamat Tinggal Kawan Kalau Balik Lagi Jangan Lupa Bawa Gorengan Jangan Lupa Subscribe Channel De Jo Bot Dan Fariz Mods dan The Ghizy Juga Biar Emaknya Bangga&language=id`)
                anunya2 =`
╭━━•› ━ ━ ━ ━ ━ ‹•━━╮
┃╭┈─────────────⩵꙰ཱི࿐
┃╰── ⏤͟͟͞GOOD BYE ──➤
...
`
                  ...
                  jobotz.sendMessage(anu.id, {audio: asa1, mimetype: 'audio/mpeg', ptt:true })
                }
            }
        } catch (err) {
            console.log(err)
